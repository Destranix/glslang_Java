...
public class Main {
	...
	static {
		try {
			NativeUtils.loadLibraryFromJar("/C/lib/"+System.mapLibraryName("SPIRV-Tools-shared"));
		} catch (IOException e) {
			try{
				System.loadLibrary("SPIRV-Tools-shared");
			}catch(UnsatisfiedLinkError e2){
				//Ignore. Include is either not necessary or Error will be thrown when loading Main
			}
		}
		try {
			NativeUtils.loadLibraryFromJar("/C/"+System.mapLibraryName("Main"));
		} catch (IOException e) {
